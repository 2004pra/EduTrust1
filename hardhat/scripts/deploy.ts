...
async function main() {
  ...
  const EduTrustCredential = await ethers.getContractFactory("EduTrustCredential");
  ...
  const X402PaymentVerifier = await ethers.getContractFactory("X402PaymentVerifier");
  const verificationFee = ethers.parseEther("0.1"); // 0.1 MON
